...
Ce workspace est le Jarvis personnel de Mohamed. Il a été créé avec le Jarvis Starter Kit pour servir d'assistant IA personnel au quotidien.
...
Je m'appelle Mohamed et je vis à Fès, au Maroc. Je suis étudiant en 5ème année de médecine générale à la Faculté de Médecine et de Pharmacie de Fès, en rotation clinique active dans plusieurs spécialités. En parallèle de mes études, je développe des compétences solides en intelligence artificielle et en software engineering pour construire MedKit, une plateforme éducative médicale propulsée par l'IA destinée aux étudiants en médecine et aux professionnels de santé.

Mes objectifs prioritaires actuels sont de lancer la première version de MedKit, de maîtriser le développement web moderne (Next.js, TypeScript, Supabase) et l'IA appliquée, et de réussir ma 5ème année tout en préparant des opportunités internationales.

À long terme, je veux devenir médecin, entrepreneur et innovateur IA, capable de construire des technologies qui améliorent l'éducation médicale et rendent le savoir médical accessible à l'échelle mondiale.

Le domaine où j'ai besoin du plus d'aide en ce moment : développement de MedKit (architecture, code, IA).
...
├── .env                         # Clés d'API privées (jamais committé)
...
├── livrables/                   # Tout ce que Claude produit (OUTPUTS)
│   ├── sites-web/               # Sites internet, landing pages, copies web
│   ├── applications/            # Outils, scripts, automatisations
│   ├── youtube/                 # Scripts, hooks, calendriers éditoriaux
│   ├── cabinet/                 # Livrables Chatflow (conseil)
│   └── ecole/                   # Livrables IApreneur Académie
├── .claude/
│   ├── commands/
...
| `context/import/` | **INPUTS** : documents externes (PDFs, exports, notes) à analyser |
| `livrables/` | **OUTPUTS** : tout ce que Claude produit pour moi |
...
**Règle d'or** : les inputs vont dans `context/import/`, les outputs vont dans `livrables/`.
